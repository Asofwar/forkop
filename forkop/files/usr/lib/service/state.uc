...
// A sing-box whose package was replaced under a running process keeps serving
// traffic, but the kernel renames its executable link to "sing-box (deleted)".
// Such a process is still ours and must stay recognizable, otherwise every
// ownership check treats the runtime as vanished and the transition stalls.
...
// procd can keep reporting an exited child for a short while after /proc no
// longer holds any sing-box executable. That is safe to wait out, but it is not
// authority to signal the reported PID: it may already have been reused. Fail
// closed if a real sing-box appears before the service state converges.
function wait_for_stale_sing_box_service_pid(timeout, transition) {
    ...
    while (timeout >= 0) {
        ...
        if (service_pid <= 0) {
            // The process scan and the ubus query are separate snapshots, so a
            // process can appear between them. Confirm the converged pair once
            // more before declaring the transition safe.
...
function stop_managed_sing_box_and_wait(timeout) {
    ...
    // No executable left, but procd still reports one: wait for that stale
    // state instead of refusing the transition outright.
    if (process_count == 0 && service_pid > 0)
