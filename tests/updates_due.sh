...
awk '
  ...
  in_update && /finish_list_update\(ok \? 0 : 1, ok, generation_changed\)/ { success_finish_line = NR }
...
awk '
  /function finish_list_update\(status, applied, generation_changed\)/ { in_finish = 1 }
