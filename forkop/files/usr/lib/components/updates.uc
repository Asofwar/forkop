...
// Keep useful source context in runtime logs without exposing subscription
// credentials, tokens, query parameters, or fragments.
...
function list_bootstrap_resolve(url) {
    ...
    for (let server in list_bootstrap_dns_servers()) {
        let pipe = fs.popen(command_from_args([ "nslookup", "-timeout=5", host, server ]) + " 2>/dev/null", "r");
        if (!pipe)
            continue;
        // BusyBox can return failure for AAAA alongside a usable A answer.
        // Keep bounded stdout and validate each answer instead of the exit code.
        let output = as_string(pipe.read(16384));
        pipe.close();
        let name_seen = false;
        let ipv6_address = "";
...
function list_curl_args(url, filepath, proxy_address, resolve) {
    let args = [
        "curl", "-f", "-sS", "--location", "--max-time", "120",
        "--connect-timeout", "20",
        "--speed-time", "15",
        "--speed-limit", "1",
        "-o", filepath
...
function finish_list_update(status, applied, generation_changed) {
    ...
    if (needs_runtime_apply) {
        ...
        // Detach stdin as well: this call is captured through a pipe, and the
        // nested init.d action must never consume or block on the worker's own
        // standard input.
        let reload_result = command_capture(command_from_args([ SERVICE_INIT, "reload", "list-content" ]) + " </dev/null 2>/dev/null" + close_procd_lock);
...
function subscription_restore_previous_runtime(backup_path, transition_timeout) {
    ...
    if (file_exists_value(backup_path) &&
...
function subscription_stop_auxiliary_runtimes() {
...
function subscription_update_common_locked(force, target_section, target_source_index) {
    ...
    if (!subscription_start_auxiliary_runtimes()) {
